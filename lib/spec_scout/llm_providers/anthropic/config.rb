...
module SpecScout
  module LLMProviders
    # Configuration class for Anthropic provider
    class AnthropicConfig
      ...
      def validate!
        raise ArgumentError, 'Anthropic API key is required' if api_key.nil? || api_key.empty?
        raise ArgumentError, 'Model must be specified' if model.nil? || model.empty?
        raise ArgumentError, 'Temperature must be between 0 and 1' unless temperature.between?(0, 1)
      ...
      # @return [AnthropicConfig] New configuration instance
