...
module SpecScout
  # Abstract base class for LLM-powered analysis optimizers
  # Provides common functionality for LLM-based optimizers that analyze profile data
  class BaseLlmOptimizer
    attr_reader :optimizer_type, :llm_provider, :context_builder, :response_parser

    def initialize(optimizer_type: nil, llm_provider: nil, context_builder: nil, response_parser: nil)
    ...
    def analyze(profile_data, spec_content = nil)
      ...
      begin
        # Build context for the AI optimizer
        context = build_context(profile_data, spec_content)

        # Get prompt template and system prompt for this optimizer type
        ...
        # Parse response into OptimizerResult
        parse_response(response)
      rescue StandardError => e
        create_error_result("LLM analysis failed: #{e.message}")
      end
    end

    # Optimizer name for identification (derived from optimizer_type)
    def optimizer_name
      @optimizer_type
    end
    ...
    # Subclasses can override to add optimizer-specific context
    ...
    def parse_response(response)
      @response_parser.parse(response, @optimizer_type)
    end

    # Get prompt template for this optimizer type
    ...
    # Get system prompt for this optimizer type
    # Subclasses can override to provide optimizer-specific system prompts
    ...
    def validate_dependencies!
      raise ArgumentError, 'optimizer_type is required' unless @optimizer_type
      ...
      # Validate optimizer_type is a symbol
      raise ArgumentError, 'optimizer_type must be a Symbol' unless @optimizer_type.is_a?(Symbol)
