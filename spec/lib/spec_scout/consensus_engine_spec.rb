...
RSpec.describe SpecScout::ConsensusEngine do
  ...
  let(:factory_optimizer_result) do
    SpecScout::OptimizerResult.new(
      optimizer_name: :factory,
  ...
  describe '#initialize' do
    it 'accepts valid optimizer results and profile data' do
      engine = described_class.new([db_optimizer_result], profile_data)
      expect(engine.optimizer_results).to eq([db_optimizer_result])
      expect(engine.profile_data).to eq(profile_data)
    end

    it 'filters out invalid optimizer results' do
      invalid_result = SpecScout::OptimizerResult.new(confidence: :invalid)
      engine = described_class.new([db_optimizer_result, invalid_result], profile_data)
      expect(engine.optimizer_results).to eq([db_optimizer_result])
    end

    it 'raises error for invalid profile data' do
      expect do
        described_class.new([db_optimizer_result], 'invalid')
      end.to raise_error(ArgumentError, 'Profile data must be a ProfileData instance')
    end
  end

  describe '#generate_recommendation' do
    context 'with no optimizer results' do
      it 'returns no action recommendation' do
        ...
        expect(recommendation.explanation).to include('No valid optimizer results available for analysis')
      end
    end

    context 'with strong agreement for optimization' do
      it 'generates strong optimization recommendation' do
        optimizer_results = [db_optimizer_result, factory_optimizer_result, risk_optimizer_result]
        engine = described_class.new(optimizer_results, profile_data)
        recommendation = engine.generate_recommendation
        ...
        expect(recommendation.agent_results).to eq(optimizer_results)
      end
    end

    context 'with high risk factors' do
      let(:high_risk_optimizer_result) do
        SpecScout::OptimizerResult.new(
          optimizer_name: :risk,
          verdict: :high_risk,
          confidence: :high,
      ...
      it 'generates no action recommendation due to high risk' do
        optimizer_results = [db_optimizer_result, factory_optimizer_result, high_risk_optimizer_result]
        engine = described_class.new(optimizer_results, profile_data)
        recommendation = engine.generate_recommendation
    ...
    context 'with conflicting optimizers' do
      ...
      it 'generates soft suggestion for conflicting persistence needs' do
        optimizer_results = [db_optimizer_result, db_required_result]
        engine = described_class.new(optimizer_results, profile_data)
        recommendation = engine.generate_recommendation

        expect(recommendation.action).to eq(:review_test_intent)
        expect(recommendation.confidence).to eq(:low)
        expect(recommendation.explanation.join(' ')).to include('Conflicting optimizer opinions detected')
      end
    end

    context 'with unclear signals' do
      let(:unclear_optimizer_result) do
        SpecScout::OptimizerResult.new(
          optimizer_name: :intent,
      ...
      it 'generates no action recommendation for unclear signals' do
        optimizer_results = [unclear_optimizer_result]
        engine = described_class.new(optimizer_results, profile_data)
        recommendation = engine.generate_recommendation

        expect(recommendation.action).to eq(:no_action)
        expect(recommendation.confidence).to eq(:low)
        expect(recommendation.explanation).to include('No clear consensus among optimizers')
      end
    end
  end

  describe 'decision matrix logic' do
    it 'applies ≥2 optimizers agree rule for strong recommendations' do
      optimizer_results = [db_optimizer_result, factory_optimizer_result]
      engine = described_class.new(optimizer_results, profile_data)
      recommendation = engine.generate_recommendation
    ...
    it 'downgrades confidence when risk optimizers flag issues' do
      potential_risk_result = SpecScout::OptimizerResult.new(
        optimizer_name: :risk,
      ...
      optimizer_results = [db_optimizer_result, factory_optimizer_result, potential_risk_result]
      engine = described_class.new(optimizer_results, profile_data)
      recommendation = engine.generate_recommendation
    ...
    it 'generates soft suggestions for conflicting optimizers' do
      conflicting_db_result = SpecScout::OptimizerResult.new(
        optimizer_name: :database,
        verdict: :db_required,
        confidence: :high,
      ...
      optimizer_results = [db_optimizer_result, conflicting_db_result]
      engine = described_class.new(optimizer_results, profile_data)
      recommendation = engine.generate_recommendation
  ...
  describe 'confidence calculation' do
    it 'calculates high confidence for strong agreement with multiple high-confidence optimizers' do
      high_conf_db = db_optimizer_result.dup.tap { |r| r.confidence = :high }
      high_conf_factory = factory_optimizer_result.dup.tap { |r| r.confidence = :high }
      high_conf_risk = risk_optimizer_result.dup.tap { |r| r.confidence = :high }

      optimizer_results = [high_conf_db, high_conf_factory, high_conf_risk]
      engine = described_class.new(optimizer_results, profile_data)
      recommendation = engine.generate_recommendation
    ...
    it 'applies risk-based confidence downgrading' do
      high_risk_result = SpecScout::OptimizerResult.new(
        optimizer_name: :risk,
        verdict: :high_risk,
        confidence: :high,
      ...
      optimizer_results = [db_optimizer_result, factory_optimizer_result, high_risk_result]
      engine = described_class.new(optimizer_results, profile_data)
      recommendation = engine.generate_recommendation
  ...
  describe 'explanation building' do
    it 'includes optimizer summary in explanation' do
      optimizer_results = [db_optimizer_result, factory_optimizer_result, risk_optimizer_result]
      engine = described_class.new(optimizer_results, profile_data)
      recommendation = engine.generate_recommendation

      # The new format includes AI/rule-based breakdown
      expect(recommendation.explanation).to include('Analyzed 3 optimizer(s): 2 high confidence, 1 medium confidence, 0 low confidence (3 rule-based optimizers)')
    end
    ...
    it 'includes consensus analysis in explanation' do
      ...
      expect(recommendation.explanation).to include('2 optimizer(s) agree on optimize_persistence approach')
    end

    it 'includes risk factors when present' do
      risk_result = SpecScout::OptimizerResult.new(
        optimizer_name: :risk,
        verdict: :potential_side_effects,
        confidence: :medium,
      ...
      optimizer_results = [db_optimizer_result, risk_result]
      engine = described_class.new(optimizer_results, profile_data)
      recommendation = engine.generate_recommendation
